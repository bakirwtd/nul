...
local correction = Instance.new("ColorCorrectionEffect", blur.Parent)

local openedtint = Color3.fromRGB(31, 39, 47)
local closedtint = Color3.new(1, 1, 1)
...
correction.TintColor = closedtint
...
lib.new = function(satamt, bluramt)
    local tbl = {}

    local nulllib = Instance.new("ScreenGui")
    nulllib.Name = "gh0stw4re"
    nulllib.Parent = game:GetService("CoreGui")
    -- nulllib.Parent = game.Players.LocalPlayer.PlayerGui
    ...
    local con2
    con2 = UserInputService.InputBegan:Connect(function(i, gs)
        if not gs then
